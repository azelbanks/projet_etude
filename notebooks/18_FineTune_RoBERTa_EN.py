#!/usr/bin/env python3
"""
18 — Fine-Tuning RoBERTa pour textes courts EN
================================================

Contexte :
    Preconisation P4 — L'anglais perd ~4% F1 entre V2 et V4, et les textes
    ultra-courts EN restent a F1=0.774 (V5). CamemBERT a prouve que les
    transformers ameliorent significativement les textes courts FR (+6.2%).
    RoBERTa-base devrait apporter un gain similaire pour EN.

Architecture :
    - Modele de base : roberta-base (125M params, pre-entraine sur 160 Go EN)
    - Fine-tuning : couches 9-11 + classification head (768 -> 256 -> 2)
    - Couches gelees : 0-8 + embeddings (~80% geles)
    - Surpoids : x2 sur les textes courts (< 30 mots)
    - Dataset : textes EN du dataset V5 (ISOT + FakeNewsNet + CONSTRAINT + Credibility EN)
...
# ============================================================
print("=" * 70)
print("FINE-TUNING RoBERTa EN — Preconisation P4")
print("=" * 70)
...
# ============================================================
#  1. CHARGEMENT DU DATASET (EN uniquement)
# ============================================================
print("\n[1/6] Chargement du dataset V5 (extraction EN)...")
...
print(f"  Textes EN total : {len(df_en)}")
print(f"  Labels : {df_en['label'].value_counts().to_dict()}")

# Stats par longueur
df_en['word_count'] = df_en['text_clean'].str.split().str.len()
for cat, mask in [('<15 mots', df_en.word_count < 15),
                  ('15-30 mots', (df_en.word_count >= 15) & (df_en.word_count < 30)),
                  ('30-100 mots', (df_en.word_count >= 30) & (df_en.word_count < 100)),
                  ('>100 mots', df_en.word_count >= 100)]:
...
df_train, df_test = train_test_split(
    df_en, test_size=0.2, stratify=df_en['label'], random_state=42
)
...
if CODECARBON_AVAILABLE:
    tracker = EmissionsTracker(
        project_name="Thumalien_RoBERTa_EN",
        output_dir=_proj,
    )
    tracker.start()
...
for epoch in range(EPOCHS):
    ...
    if val_f1 > best_val_f1:
        ...
        }, os.path.join(MODEL_DIR, 'roberta_en.pt'))
...
checkpoint = torch.load(os.path.join(MODEL_DIR, 'roberta_en.pt'), map_location=device, weights_only=True)
base_model.load_state_dict(checkpoint['base_model_state'])
head.load_state_dict(checkpoint['head_state'])
logger.info("RoBERTa best checkpoint recharge (F1=%.4f)", best_val_f1)
...
print(f"\n  Score test EN : {correct}/{len(test_en)}")

# ============================================================
#  6. SAUVEGARDE
# ============================================================
print(f"\n[6/6] Sauvegarde RoBERTa EN...")
print(f"  Sauvegarde dans {os.path.join(MODEL_DIR, 'roberta_en.pt')}")

elapsed = time.time() - t0
print(f"\n{'=' * 70}")
print(f"Fine-tuning RoBERTa EN termine en {elapsed:.0f}s ({elapsed/60:.1f} min)")
print(f"{'=' * 70}")
