...
def compute_fairness_metrics(texts, labels_true, languages, text_lengths, detector):
    ...
    metrics = {"timestamp": datetime.now(UTC).isoformat(), "n_total": len(texts), "groups": {}}
...
if __name__ == "__main__":
    ...
    logger.info(
        "This module requires labeled test data. Use compute_fairness_metrics() programmatically."
    )
    logger.info(
        "Example: compute_fairness_metrics(texts, true_labels, languages, word_counts, detector)"
    )
