...
class DatasetCleaner:
    ...
    def prepare_bilingual_dataset(
        ...
        **kwargs,
        ...
        if french_oversample > 1:
            df_fr = pd.concat([df_fr] * french_oversample, ignore_index=True)
        ...
        if social_parts:
            ...
            if social_oversample > 1:
                df_social = pd.concat([df_social] * social_oversample, ignore_index=True)
            logger.info(
    ...
    def audit_reuters_leakage(cls, df_true: pd.DataFrame) -> dict:
        ...
        has_city_dash = texts.str.contains(r"^[A-Z]{2,}.*[-–—]", regex=True).sum()
        has_byline = texts.str.contains(r"Reporting by|Editing by", case=False).sum()
    ...
    def load_fakenewsnet(
        ...
        if not dfs:
            raise FileNotFoundError(f"Aucun fichier FakeNewsNet trouvé dans {data_dir}")
    ...
    def load_constraint(
        ...
        if not dfs:
            raise FileNotFoundError(f"Aucun fichier CONSTRAINT trouvé dans {data_dir}")
    ...
    def load_credibility_corpus(
        ...
        if not dfs:
            raise FileNotFoundError(f"Aucun fichier Credibility Corpus trouvé dans {data_dir}")
