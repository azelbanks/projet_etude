...
class ExpertFakeNewsDetector:
    ...
    def train(
        ...
        try:
            ...
            X_text_original = df["text_original"].values if "text_original" in df.columns else None
            y = df["label"].values
            ...
            if bilingual:
                ...
                lang_weight_map = {
                    lang: total / (n_langs * count) for lang, count in lang_counts.items()
                }
            ...
            self.training_metrics = {
                ...
                "n_features_emotion": len(EmotionFeatureExtractor.FEATURE_NAMES)
                ...
                "cv_f1_per_fold": [round(float(x), 4) for x in cv_results["test_f1"]],
            }
    ...
    def evaluate_holdout(self, df: pd.DataFrame) -> dict:
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            results["roc_auc"] = round(float(roc_auc_score(y_true, y_proba)), 4)
    ...
    def predict(self, texts: pd.Series, track_emissions: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            results["ai_analysis_log"] = results.apply(lambda r: self._make_log(r), axis=1)
    ...
    def predict_adaptive(self, texts: pd.Series, track_emissions: bool = False) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if getattr(self, "_roberta_en", None) is not None:
                en_short_mask = ((results["language"] == "en") & (word_counts < 30)).values
                if en_short_mask.any():
                    ...
                    try:
                        ...
                        logger.info(
                            "V9 cascade: blended %d short EN texts with RoBERTa",
                            en_short_mask.sum(),
                        )
    ...
    def load(self, suffix: str = "expert") -> bool:
        ...
        self.model = joblib.load(os.path.join(self.model_dir, f"model_{suffix}.pkl"))
        self.vectorizer = joblib.load(os.path.join(self.model_dir, f"tfidf_{suffix}.pkl"))
