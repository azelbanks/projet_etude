...
class TestNormalizeMongoDf:
    ...
    def test_label_suspect_int(self):
        df = pd.DataFrame(
            {"text": ["test"], "prediction_label": [1], "ai_score_credibility": [0.2]}
        )
        result = app._normalize_mongo_df(df)
    ...
    def test_v9_label_takes_priority(self):
        df = pd.DataFrame(
            {"text": ["t"], "ai_v9_label": ["SUSPECT"], "prediction_label": ["FIABLE"]}
        )
        result = app._normalize_mongo_df(df)
        assert result["prediction_label"].iloc[0] == "SUSPECT"
    ...
    def test_datetime_parsing(self):
        df = pd.DataFrame(
            {"text": ["t"], "prediction_label": [0], "collected_at": ["2026-01-01T10:00:00"]}
        )
        result = app._normalize_mongo_df(df)
...
class TestPredictV7Hybrid:
    ...
    def test_with_camembert_meta(self):
        ...
        v7_data = {
            ...
            "uses_camembert": True,
            "version": "v8_meta",
            "camembert_suffix": "camembert_fr",
        }

        cam = MagicMock()
        cam.predict_credibility_scores.return_value = [0.9]

        result = app.predict_v7_hybrid(
            "test text", detector, emo, v6_data, v7_data, cam_classifier=cam
        )
