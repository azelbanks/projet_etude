...
class TestStyleFeatureExtractorV6:
    ...
    def test_extract_returns_correct_length(self):
        ...
        assert result.shape[1] == 28, f"Expected 28 features, got {result.shape[1]}"
    ...
    def test_has_source_citation(self):
        ...
        texts = pd.Series(["According to Reuters, the new policy will take effect next month."])
        result = StyleFeatureExtractorV6.extract(texts)
    ...
    def test_suspect_vs_fiable_features_differ(self):
        ...
        assert feat_s[0, sens_idx] > feat_f[0, sens_idx], (
            "Suspect should have higher sensationalism"
        )
