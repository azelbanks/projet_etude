...
def trained_detector():
    ...
    d.vectorizer.get_feature_names_out.return_value = np.array([f"word_{i}" for i in range(20000)])
...
class TestPredict:
    ...
    def test_predict_with_lang_thresholds(self, trained_detector_lang_thresholds, sample_texts):
        result = trained_detector_lang_thresholds.predict(sample_texts, track_emissions=False)
        assert isinstance(result, pd.DataFrame)
        assert len(result) == 2
...
class TestPredictAdaptive:
    ...
    def test_predict_adaptive_short_text_threshold(self, trained_detector):
        ...
        trained_detector.vectorizer.transform.return_value = csr_matrix(np.random.rand(1, 20000))
...
class TestEvaluateHoldout:
    def test_evaluate_holdout_returns_metrics(self, trained_detector):
        ...
        trained_detector.model.predict_proba.return_value = np.array([[0.8, 0.2], [0.3, 0.7]])
    ...
    def test_evaluate_holdout_has_roc_auc(self, trained_detector):
        ...
        trained_detector.model.predict.return_value = np.array([0, 1])
        trained_detector.model.predict_proba.return_value = np.array([[0.9, 0.1], [0.2, 0.8]])
        result = trained_detector.evaluate_holdout(df)
...
class TestExplainPrediction:
    ...
    def test_explain_has_sensationalist_words(self, trained_detector):
        ...
        result = trained_detector.explain_prediction("SHOCKING conspiracy exposed! Wake up people!")
        assert result["explainable"] is True
    ...
    def test_explain_with_lang_thresholds(self, trained_detector_lang_thresholds):
        ...
        d.vectorizer.transform.return_value = csr_matrix(np.random.rand(1, 20000))
...
class TestBuildFeatures:
    ...
    def test_build_features_with_original_texts(self, trained_detector):
        ...
        X = trained_detector._build_features(texts_clean, texts_original=texts_original, fit=True)
        assert X.shape[0] == 2
