...
Le pipeline de collecte (`collect_bluesky.py`, ~470 lignes) est fonctionnel et a permis de collecter plus de 537 000 posts (collecte continue jusqu'au 19 mai 2026). Il utilise l'API AT Protocol avec authentification, recherche par 210+ mots-clés bilingues (FR + EN), stockage MongoDB avec deduplication par `bulk_write`, resilience par backoff exponentiel, et inference IA automatique a chaque cycle (emotions + V5 + Stage1 V9).
...
L'entrainement cumule consomme 8.86 g CO2 (dont 64% pour les Transformers CamemBERT/RoBERTa), mais combien consomme l'inference quotidienne sur 2 000 posts ? Sur une annee, l'inference cumule probablement plus que l'entrainement.
